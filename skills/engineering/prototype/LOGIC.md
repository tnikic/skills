...
Because it's one file with nothing to install, you can hand it to a non-developer (a designer, a PM, or a domain expert) and let them feel the model for themselves. Write in domain language, not code language.
...
*Completion: the demo states the question and the state model in a visible introductory paragraph.*
...
Pick whichever shape best fits the question being asked, *not* whichever is easiest to wire to a page. Keep it pure: no DOM, no `document`, and no button handlers reaching inside it. The page calls into it; nothing flows the other direction.
...
*Completion: the logic is isolated from DOM and event-handler code behind a portable interface.*
...
1. **Title and one-line explanation** of what this demo lets someone explore, using the question from step 1.
2. **Current state**: the full relevant state, rendered as a readable panel with labelled fields, not a raw JSON dump. Re-render after every click so the change is visible. Where it helps someone follow, call out what just changed.
...
4. **Guided walkthroughs**: a set of scenarios, one per tab. Each tab has a short plain-language description and ordered buttons for that scenario. Starting a walkthrough resets to a known initial state so the scenario runs the same way every time.

Choose scenarios that demonstrate awkward cases: the happy path, a tricky edge case, and an attempt at something that should be illegal.

Keep it beautiful but restrained: clean typography, generous spacing, one accent colour. No animations or gimmicks that compete with the state and buttons.

*Completion: the file opens without installation and exposes free-play state plus repeatable guided scenarios.*

### 4. Hand it over

Send the user the file, or open it for them. They'll click through the walkthroughs and free-play whenever they get to it. The interesting moments are when they say "wait, that shouldn't be possible" or "huh, I assumed X would be different". Those are the bugs in the idea.
