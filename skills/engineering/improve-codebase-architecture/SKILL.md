...
- The domain language in `docs/CONTEXT.md` gives names to good seams; ADRs in `docs/adr/` record decisions this command should not re-litigate.
...
Read the project's domain glossary (`docs/CONTEXT.md`) and any ADRs in the area you're touching first.

Then use the `subagent` tool with `agent: "architect"` to walk the codebase. Don't follow rigid heuristics — explore organically and note where you experience friction:
...
**Use `docs/CONTEXT.md` vocabulary for the domain, and the `/codebase-design` vocabulary for the architecture.** If `docs/CONTEXT.md` defines "Order," talk about "the Order intake module" — not "the FooBarHandler," and not "the Order service."
...
- **Resolving a test-gap candidate?** When the project agrees on the missing test types and a plan to add them, add the agreed test types as targets in the project's Makefile or justfile. Use the [test-type taxonomy](../../shared/test-types.md) for vocabulary.
